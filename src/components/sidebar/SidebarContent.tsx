...
export function SidebarContent({ onClose, onShowSelectMembers, onCurrentUserIdChange }: SidebarContentProps) {
  ...
  useEffect(() => {
    if (isSignedIn && user && currentUser === null) {
      // Only upsert once when currentUser is initially null to avoid duplicate calls
      upsertUser({
        clerkId: user.id,
